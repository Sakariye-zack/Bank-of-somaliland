...
const MAX_DOCUMENT_BYTES = 30 * 1024 * 1024; // 30MB
...
function dirForField(fieldname: string): string {
  if (fieldname === 'video') return VIDEOS_DIR;
  if (fieldname === 'document') return DOCUMENTS_DIR;
  return IMAGES_DIR;
}

const storage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, dirForField(file.fieldname)),
...
export const uploadMedia = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (file.fieldname === 'document' && ALLOWED_DOCUMENT_TYPES.has(file.mimetype)) return cb(null, true);
...
export function enforcePerFieldSizeLimits(files: UploadedFields): string | null {
  ...
  for (const f of files.document ?? []) {
    if (f.size > MAX_DOCUMENT_BYTES) return `Document "${f.originalname}" exceeds the 30MB limit.`;
  }
  return null;
}

export function cleanupFiles(files: UploadedFields) {
  for (const f of [...(files.images ?? []), ...(files.video ?? []), ...(files.document ?? [])]) {
    fs.unlink(f.path, () => {});
  }
}

export function publicUrlFor(file: Express.Multer.File): string {
  const kind = file.fieldname === 'video' ? 'videos' : file.fieldname === 'document' ? 'documents' : 'images';
